...
$date_of_birth = new DateTime($date_of_birth); // Convert to DateTime object
$date_of_birth_formatted = $date_of_birth->format('Y-m-d'); // Format as YYYY-MM-DD
 ...
 // Prepare the SQL query using a prepared statement
$sql = "INSERT INTO users (firstname, lastname, phone, gender, email, address, province, password, date_of_birth) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = mysqli_prepare($conn, $sql);

// Bind parameters including the date of birth
mysqli_stmt_bind_param($stmt, "sssssssss", $firstname, $lastname, $phone, $gender, $email, $address, $province, $hashed_password, $date_of_birth_formatted);
